...
echo $($JAVA_HOME/bin/java --version) > $OUTPUT_FOLDER/test-details.txt

time_start=$(date +%s)

# Run the command
$1 > $OUTPUT_FOLDER/app.log &
MY_PID=$!
...
# sleep until ready
while [[ "$(curl -s -o /dev/null -w ''%{http_code}'' $2)" != "200" ]]; do sleep 0.5; done
...
time_start=$(date +%s)
ab -q -c 5 -n 5000 $URL_TEST > $OUTPUT_FOLDER/ab_output.txt
time_end=$(date +%s)
time_spent=$((($time_end - $time_start)))
